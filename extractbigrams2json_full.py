...
'''
with open('relationFreqdict.tsv', 'r') as relations:
    relations = relations.readlines()
    
dicttt = {k:v for k,v in (x.split(':') for x in relations) }
#print(dicttt)
dicttt = { k:int(v) for k,v in dicttt.items()}
...
for root, dirs, files in os.walk(r'E:\NewSketch\newpy2\corpus'):
    ...
    for relation in bigrams[word]:
        for lemma in bigrams[word][relation][1]:
            try:
                ...
                logLem = 0
            bigrams[word][relation][1][lemma][1] = logLem
            for tag in bigrams[word][relation][1][lemma][2]:
                try:
                    logTok = logDice(bigrams, word, relation, lemma, tag)
                except:
                    logTok = 0
                bigrams[word][relation][1][lemma][2][tag][2] = logTok


with open('bigrams.json', 'a') as b:
